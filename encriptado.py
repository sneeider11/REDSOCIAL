...
def registrar_usuario():
    ...
    nuevo_usuario_ref = usuarios_ref.push({
        "email": datos["email"],
        "nombre": datos["nombre"],
        "contraseña": hashed_password.decode('utf-8'),  # Almacenar el hash como string
        "activo": True
    })
...
def iniciar_sesion():
    ...
    for uid, user_data in usuarios.items():
        if user_data.get('email') == credenciales["email"]:
            if bcrypt.checkpw(credenciales["contraseña"].encode('utf-8'), user_data.get('contraseña').encode('utf-8')):
